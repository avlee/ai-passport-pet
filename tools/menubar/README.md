...
- **python**: switches the interpreter the bridge runs on, and the item title states whether that interpreter has Pillow. **Pet swapping packs the atlas into a `.pet` with it**, so an interpreter without Pillow still runs the bridge fine and only breaks pet swapping — a "half working" failure that is hard to trace, which is why the pets submenu states the reason up front instead. Clearing the field returns to auto-detection (`/opt/homebrew`, `/usr/local`, `/usr/bin`, preferring whichever can `import PIL`).
