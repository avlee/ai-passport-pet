...
- **python**：换桥接用的解释器，标题里直接标出它有没有 Pillow。**换宠物要靠它把图集**
  **切成 `.pet`**，所以没 Pillow 解释器时桥接本体照常跑，只有换宠物会失败 —— 那种「一半
  能用」最难查，于是「宠物」子菜单会在点之前就把原因写在顶上。留空则回到自动探测（按
  `/opt/homebrew`、`/usr/local`、`/usr/bin` 的顺序挑，优先挑能 `import PIL` 的那个）。
