// main/demo_menu.c —— FoloToy AI Passport BSP 驱动参考示例的菜单。
//
// 这段代码原本就是 main.c 的全部内容(初始化 + 菜单 + 按键分发), 现在原样搬过来,
// 只做两处必要改动:
//   1. 按键队列由 pet_app.c 统一创建, 这里只负责消费;
//   2. 主菜单上长按「确定」= 退出菜单, 把屏幕交还给 Codex 宠物界面。
//
// 按键语义(与原来一致):
//   上/下 短按   菜单中=移动选中项;演示页中=该页自定义
//   确定  短按   菜单中=进入选中项;演示页中=该页自定义
//   确定  长按   演示页中=返回菜单;菜单中=退出并回到宠物界面
#include "demo_menu.h"
...
static const char *TAG = "demo_menu";
...
// 每项只探测一次 —— 菜单可以在运行期反复进出, 反复 init 会重复申请资源。
...
static demo_nav_input_t navigation_input(const app_input_t *input)
{
    if (input->event == BSP_BTN_LONG && input->btn == BSP_BTN_OK) {
        return DEMO_NAV_INPUT_OK_LONG;
    }
    if (input->event != BSP_BTN_CLICK) return DEMO_NAV_INPUT_OTHER;
    if (input->btn == BSP_BTN_UP) return DEMO_NAV_INPUT_UP_CLICK;
    if (input->btn == BSP_BTN_DOWN) return DEMO_NAV_INPUT_DOWN_CLICK;
    if (input->btn == BSP_BTN_OK) return DEMO_NAV_INPUT_OK_CLICK;
    return DEMO_NAV_INPUT_OTHER;
}

static void process_input(const app_input_t *input)
{
    const demo_nav_input_t nav_input = navigation_input(input);

    // 主菜单上长按「确定」= 离开演示菜单。演示页里长按确定由下面的分支处理成
    // 「返回菜单」, 所以要退两次, 语义与原来保持一致。
...
static void probe_peripherals(void)
{
    ...
    ESP_LOGI(TAG, "外设探测: Audio=%d Battery=%d", s_ok[2], s_ok[3]);
}
...
void demo_menu_run(QueueHandle_t input_queue)
{
    ...
    if (!bsp_lvgl_lock(1000)) {
        ESP_LOGE(TAG, "拿不到 LVGL 锁, 演示菜单未打开");
        return;
    }
    menu_build();
    bsp_lvgl_unlock();
    ...
    while (!s_exit_requested) {
        if (!app_input_receive(input_queue, &input, 200)) continue;
        process_input(&input);
    }

    // 用户没进任何演示页时菜单屏还在, 需要显式删掉。
    if (bsp_lvgl_lock(1000)) {
        if (s_menu_scr != NULL) {
            lv_obj_delete(s_menu_scr);
            s_menu_scr = NULL;
