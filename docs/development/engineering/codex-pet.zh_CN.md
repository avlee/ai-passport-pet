...
同一条记录里还有 `plan_type`（`plus`、`pro`…）。桥接把它作为**只走控制通道**的额外字段
发出去 —— `{"event":"limits",…,"plan":"plus"}` —— 菜单栏渲染成 `订阅：Plus` 一行。刻意
不下发设备：两条能量槽没地方放它，而设备协议不动就等于固件不用改。值原样透传（只做去空白、
限长与字符检查），首字母大写之类的展示格式化归菜单栏 —— 以后官方加新档位不必改代码。
...
生长，剩余量低于 50%/20% 时颜色依次转为黄/红。`tools/preview_pet_screen.py --limits 98/31`
不用真机就能出预览图。
