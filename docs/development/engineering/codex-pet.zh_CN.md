...
- 顶部一行：左侧状态圆点 + 状态文字（离线 / 连接中 / 空闲 / 工作中 / 等待确认 / 已完成 / 出错了），右侧电量。状态文字用 20px 标题档、电量数字用 16px 正文档 —— 这一档大小差是有意的，让「它在干什么」比「还剩多少电」先被读到。两者不靠行框等高对齐（字号不同，行框永远等不了高）：`STATUS_Y` 与 `BAT_TEXT_Y` 各自让**墨迹中心**落在电池图形的中线（y=20）上。
- 电量下面一颗圆角徽标，写订阅档位（`Plus`）—— 只在桥接读到过档位时出现：底色是那抹绿的低透明度，边框与电量同一个绿。还没读到档位、或宠物宽到占住右上角时，整块隐藏。
...
Codex 没有本地查询订阅用量的命令，但服务端随每个模型响应返回限额快照，Codex 会把这些快照
写进会话 rollout（`event_msg/token_count` 记录）。桥接读取后转发
`{"type":"limits","primary":<0-100>,"weekly":<0-100>}` —— 分别是 5 小时窗（`primary`，
300 分钟）与周窗（`secondary`，10080 分钟）的**已用**百分比。某个窗口没有可用快照时整个
字段不下发；两个窗口都没有的行会被解析器丢弃。

同一条记录里还有 `plan_type`（`plus`、`pro`…）。桥接把值原样透传（只做去空白、限长与字符检查），
往两个面上各发一份：控制通道的 `{"event":"limits",…,"plan":"plus"}`，以及设备消息里的
`{"type":"limits",…,"plan":"plus"}` —— 固件把后者画成电量下面那个圆角徽标。首字母大写不是桥接的事：
每个展示面自己格式化，设备侧是 `pet_plan_label()`，菜单栏侧是 `codexPlanLabel`，以后官方加新档位不必
改代码。还没读到档位、或者宠物宽到右上角本来就属于它时（几何见 `main/pet_ui.c`），徽标整块隐藏；徽标
字体是只有拉丁字形的内建 Montserrat 12，所以非 ASCII 字节直接丢掉，而不是画成缺字方框。

发布 `limits` 的两条路径 —— 实时跟随流与 30 秒回扫 —— 必须往事件里放**同样的字段**。二者
共用控制通道里同一个槽位，字段少的那条会静默覆盖字段多的；这也是 `send_limits()` 收整份
快照而不是收两个裸数字的原因。

过期数据两边都有处理：

- 桥接每 30 秒回扫最近的 rollout，`resets_at` 已过的窗口按已用 0% 报 —— 窗口已经翻页，
  翻页后若有请求必然写出新快照。
- 设备把没有值的窗口藏起来，掉线时两条能量槽一起隐藏（旧数据画上去就是骗人）。数值有缓存，
  重建界面（演示菜单、换宠物）后立即恢复；设备重连后桥接会补发一份。

屏幕上两个窗口是贴着左右边缘的竖形能量槽（左 `5h`、右 `7d`），填充从底部按**剩余**百分比
生长。两条槽各自固定颜色（5 小时窗红、周窗蓝），不随剩余量分级 —— 颜色、标签、位置三者
合起来才是区分依据（分级变色的是电量，不是能量槽）。`tools/preview_pet_screen.py --limits 98/31`
不用真机就能出预览图。
...
- 屏幕上显示 4 位配对码。它**每次开窗都换**：打开配网界面就生成一个新码并存 NVS（`pet_provision.c` 的 `rotate_pin()`），同一窗口内重连沿用，窗口关了就作废 —— 串口日志和 NVS 里的旧码从此配不上对。连续错满 `PET_PROVISION_MAX_ATTEMPTS` 次也会立刻换码并断开链路 —— 那已经不是手滑，而是有人在试。换码对用户没有额外成本：配网本来就要看屏幕抄码。
- 尝试次数**不随连接重置**。每次开窗换新码、次数随之回到上限；但同一窗口内重连不白送次数 —— 「沿用同一个码 + 每次连接白送三次」等于可以把码慢慢试出来。
